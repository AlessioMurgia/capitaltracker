  ...
  <div class="font-sans w-full">
    ...
    <div class="max-w-screen-xl mx-auto p-4 md:p-6 lg:p-8">
      ...
      <header class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
        <div class="grow">
          <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">Asset Valuations</h1>
          <p class="text-slate-500 dark:text-slate-400 mt-1">Monitor automated valuations and manage manual entries.</p>
        </div>
        <div class="w-full md:w-64">
          <Select v-model="selectedPortfolioId">
            <SelectTrigger class="w-full md:w-[200px] bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 h-11">
      ...
      <div v-if="isLoading" class="animate-pulse space-y-8">
        <div class="h-12 bg-slate-100 dark:bg-slate-800/50 rounded-lg w-1/3"></div>
      ...
      <div v-else class="grid grid-cols-1 gap-8">
        <Card class="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-xl">
          <CardHeader>
            <CardTitle class="text-slate-900 dark:text-white text-2xl">Manual Valuations</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <Accordion v-else type="single" collapsible class="w-full">
              <AccordionItem v-for="asset in manualTrackedAssets" :key="asset.id" :value="asset.id" class="border-b border-slate-200 dark:border-slate-700/60">
                <AccordionTrigger class="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-md text-left">
                  <div class="flex justify-between items-center w-full">
                    <div>
                      <p class="font-semibold text-slate-800 dark:text-slate-200">{{ asset.name }}</p>
                      <p class="text-sm text-slate-500 dark:text-slate-400">{{ asset.asset_class }}</p>
                    </div>
                    <div class="text-right">
                      <p class="text-lg font-bold text-slate-900 dark:text-white">€{{ (asset.valuations[0]?.value || 0).toLocaleString('it-IT', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}</p>
                      <p class="text-xs text-slate-500">Latest Value</p>
                    </div>
                  </div>
                </AccordionTrigger>
                <AccordionContent class="p-4 bg-slate-50 dark:bg-slate-900/50">
                  ...
                  <Table>
                    <TableHeader><TableRow class="border-b-slate-200 dark:border-b-slate-700/60 hover:bg-slate-100 dark:hover:bg-slate-800/50"><TableHead class="text-slate-900 dark:text-white">Date</TableHead><TableHead class="text-right text-slate-900 dark:text-white">Value</TableHead><TableHead class="text-right text-slate-900 dark:text-white">Actions</TableHead></TableRow></TableHeader>
                    <TableBody>
                      <TableRow v-for="val in asset.valuations" :key="val.id" class="border-b-slate-100 dark:border-b-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800">
                        <TableCell class="text-slate-800 dark:text-slate-300">{{ new Date(val.date).toLocaleDateString() }}</TableCell>
                        <TableCell class="text-right font-medium font-mono text-slate-800 dark:text-slate-300">€{{ val.value.toLocaleString('it-IT', {minimumFractionDigits: 2}) }}</TableCell>
                        <TableCell class="text-right">
                          <Button variant="ghost" size="icon" @click="openEditValuationDialog(val)" class="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-700"><Pencil class="h-4 w-4" /></Button>
                          <Button variant="ghost" size="icon" @click="openDeleteValuationDialog(val)" class="text-slate-500 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-slate-200 dark:hover:bg-slate-700"><Trash2 class="h-4 w-4" /></Button>
                        </TableCell>
                      </TableRow>
                    </TableBody>
                  </Table>
        ...
        <Card class="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-xl">
          <CardHeader>
            <CardTitle class="text-slate-900 dark:text-white text-2xl">Auto-Tracked Valuations</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div v-else>
              <Table>
                <TableHeader><TableRow class="border-b-slate-200 dark:border-b-slate-700/60 hover:bg-slate-50 dark:hover:bg-slate-800/50"><TableHead class="text-slate-900 dark:text-white">Asset Name</TableHead><TableHead class="text-slate-900 dark:text-white">Asset Class</TableHead><TableHead class="text-slate-900 dark:text-white">Last Updated</TableHead><TableHead class="text-right text-slate-900 dark:text-white">Latest Value</TableHead></TableRow></TableHeader>
                <TableBody><TableRow v-for="asset in autoTrackedAssets" :key="asset.id" class="border-b-slate-100 dark:border-b-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800">
                  ...
                  <TableCell class="text-slate-600 dark:text-slate-400">{{ asset.valuations[0] ? new Date(asset.valuations[0].date).toLocaleDateString() : 'N/A' }}</TableCell>
                  <TableCell class="text-right font-medium font-mono text-slate-800 dark:text-slate-300">
      ...
      <Dialog :open="isAddDialogOpen" @update:open="isAddDialogOpen = $event">
        <DialogContent class="sm:max-w-[425px] bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200">
          <DialogHeader><DialogTitle class="text-slate-900 dark:text-white">Add Valuation for {{ valuationToAdd.asset_name }}</DialogTitle><DialogDescription class="text-slate-500 dark:text-slate-400">Enter the new value and the date of the valuation.</DialogDescription></DialogHeader>
          <div class="grid gap-4 py-4">
            <div class="grid grid-cols-4 items-center gap-4"><label for="date-add" class="text-right text-slate-500 dark:text-slate-400">Date</label><Input id="date-add" type="date" v-model="valuationToAdd.date" class="col-span-3 bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" /></div>
            <div class="grid grid-cols-4 items-center gap-4"><label for="value-add" class="text-right text-slate-500 dark:text-slate-400">Value (€)</label><Input id="value-add" type="number" v-model="valuationToAdd.value" class="col-span-3 bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" /></div>
          </div>
          <DialogFooter>
      ...
      <Dialog :open="isEditDialogOpen" @update:open="isEditDialogOpen = $event">
        <DialogContent class="sm:max-w-[425px] bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200">
          <DialogHeader><DialogTitle class="text-slate-900 dark:text-white">Edit Valuation</DialogTitle><DialogDescription class="text-slate-500 dark:text-slate-400">Update the value or date for this historical record.</DialogDescription></DialogHeader>
          <div v-if="valuationToEdit" class="grid gap-4 py-4">
            <div class="grid grid-cols-4 items-center gap-4"><label for="date-edit" class="text-right text-slate-500 dark:text-slate-400">Date</label><Input id="date-edit" type="date" v-model="valuationToEdit.date" class="col-span-3 bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" /></div>
            <div class="grid grid-cols-4 items-center gap-4"><label for="value-edit" class="text-right text-slate-500 dark:text-slate-400">Value (€)</label><Input id="value-edit" type="number" v-model="valuationToEdit.value" class="col-span-3 bg-slate-100 dark:bg-slate-700 border-slate-300 dark:border-slate-600" /></div>
          </div>
          <DialogFooter>
