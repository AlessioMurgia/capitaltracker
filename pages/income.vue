  ...
  <div class="font-sans w-full">
    ...
    <div class="max-w-screen-xl mx-auto p-4 md:p-6 lg:p-8">
      ...
      <header class="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4">
        <div class="grow">
          <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight text-slate-900 dark:text-white">Income</h1>
          <p class="text-slate-500 dark:text-slate-400 mt-1">Track your upcoming and historical investment payouts.</p>
        </div>
        <div class="flex items-center gap-4 w-full md:w-auto">
          <Select v-model="selectedPortfolioId">
            <SelectTrigger class="w-full md:w-[200px] bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 h-11"><SelectValue placeholder="All Portfolios" /></SelectTrigger>
      ...
      <div v-if="isLoading" class="animate-pulse space-y-4">
        <div class="h-10 bg-slate-100 dark:bg-slate-800/50 rounded-lg w-1/4"></div>
      ...
      <Tabs v-else default-value="upcoming" class="w-full">
        <TabsList class="bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60">
          <TabsTrigger value="upcoming" class="text-slate-600 dark:text-slate-400 data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700 data-[state=active]:text-slate-900 dark:data-[state=active]:text-white">Upcoming</TabsTrigger>
          <TabsTrigger value="paid" class="text-slate-600 dark:text-slate-400 data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700 data-[state=active]:text-slate-900 dark:data-[state=active]:text-white">Paid History</TabsTrigger>
        </TabsList>
        <TabsContent value="upcoming" class="mt-6">
          <Card class="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-xl">
            <CardHeader><CardTitle class="text-slate-900 dark:text-white">Upcoming Payouts</CardTitle></CardHeader>
            <CardContent>
              <Table>
                <TableHeader><TableRow class="border-b-slate-200 dark:border-b-slate-700/60 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <TableHead class="text-slate-900 dark:text-white">Date</TableHead>
                  <TableHead class="text-slate-900 dark:text-white">Source</TableHead>
                  <TableHead class="text-slate-900 dark:text-white">Description</TableHead>
                  <TableHead class="text-right text-slate-900 dark:text-white">Amount</TableHead>
                  <TableHead class="text-center text-slate-900 dark:text-white">Actions</TableHead>
                ...
                <TableBody>
                  ...
                  <TableRow v-for="payout in upcomingPayouts" :key="payout.id" class="border-b-slate-100 dark:border-b-slate-800" :class="{ 'bg-yellow-100/50 dark:bg-yellow-900/20 border-l-4 border-yellow-500': !payout.destination_asset_id }" :title="!payout.destination_asset_id ? 'Cash account needed to transfer transaction' : ''">
                    <TableCell class="text-slate-800 dark:text-slate-300">{{ new Date(payout.payout_date).toLocaleDateString() }}</TableCell>
                    <TableCell class="text-slate-800 dark:text-slate-300">{{ payout.source_asset?.name || 'Manual Income' }}</TableCell>
                    <TableCell class="text-slate-600 dark:text-slate-400">{{ payout.description }}</TableCell>
                    <TableCell class="text-right font-semibold text-green-500 dark:text-green-400 font-mono">+€{{ payout.amount.toLocaleString('it-IT', {minimumFractionDigits: 2}) }}</TableCell>
                    <TableCell class="text-center">
                      <div class="flex items-center justify-center gap-1">
                        <Button v-if="!payout.destination_asset_id" @click="openAssignDialog(payout)" size="sm" variant="outline" class="h-8 border-yellow-500/50 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 hover:bg-yellow-200 dark:hover:bg-yellow-900/50">
                          <AlertTriangle class="h-4 w-4 mr-2" />
                          Assign
                        </Button>
                        <template v-if="payout.source_type === 'MANUAL'">
                          <Button @click="openEditDialog(payout)" size="icon" variant="ghost" class="h-8 w-8 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-700"><Pencil class="h-4 w-4" /></Button>
                          <Button @click="openDeleteDialog(payout)" size="icon" variant="ghost" class="h-8 w-8 text-slate-500 dark:text-slate-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-slate-100 dark:hover:bg-slate-700"><Trash2 class="h-4 w-4" /></Button>
                        </template>
                      </div>
                    </TableCell>
                  </TableRow>
        ...
        <TabsContent value="paid" class="mt-6">
          <Card class="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/60 rounded-xl">
            <CardHeader><CardTitle class="text-slate-900 dark:text-white">Paid Payouts</CardTitle></CardHeader>
            <CardContent>
              <Table>
                <TableHeader><TableRow class="border-b-slate-200 dark:border-b-slate-700/60 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <TableHead class="text-slate-900 dark:text-white">Date</TableHead>
                ...
                <TableBody>
                  ...
                  <TableRow v-for="payout in paidPayouts" :key="payout.id" class="border-b-slate-100 dark:border-b-slate-800">
                    <TableCell class="text-slate-500 dark:text-slate-400">{{ new Date(payout.payout_date).toLocaleDateString() }}</TableCell>
                    <TableCell class="text-slate-500 dark:text-slate-400">{{ payout.source_asset?.name || 'Manual Income' }}</TableCell>
                    <TableCell class="text-slate-600 dark:text-slate-500">{{ payout.description }}</TableCell>
                    <TableCell class="text-right font-medium text-slate-800 dark:text-slate-300 font-mono">€{{ payout.amount.toLocaleString('it-IT', {minimumFractionDigits: 2}) }}</TableCell>
                    <TableCell class="text-center">
      ...
      <Dialog :open="isAddDialogOpen" @update:open="isAddDialogOpen = $event">
        <DialogContent class="sm:max-w-md bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200">
          <DialogHeader><DialogTitle class="text-slate-900 dark:text-white">{{ dialogTitle }}</DialogTitle></DialogHeader>
          <div class="grid gap-4 py-4 max-h-[70vh] overflow-y-auto px-1">
            <div class="space-y-2"><Label class="text-slate-600 dark:text-slate-400">Description</Label><Input v-model="payoutToEdit.description" placeholder="e.g., Rent for July" class="bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600"/></div>
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-2"><Label class="text-slate-600 dark:text-slate-400">Amount (€)</Label><Input type="number" v-model="payoutToEdit.amount" class="bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600"/></div>
              <div class="space-y-2"><Label class="text-slate-600 dark:text-slate-400">Payout Date</Label><Input type="date" v-model="payoutToEdit.payout_date" class="bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600"/></div>
            </div>
            <div class="space-y-2"><Label class="text-slate-600 dark:text-slate-400">Source Asset (Optional)</Label><Select v-model="payoutToEdit.source_asset_id"><SelectTrigger class="bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600"><SelectValue placeholder="Select source asset" /></SelectTrigger><SelectContent class="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200"><SelectGroup><SelectItem v-for="a in assetsList" :key="a.id" :value="a.id">{{ a.name }}</SelectItem></SelectGroup></SelectContent></Select></div>
            <div class="space-y-2"><Label class="text-slate-600 dark:text-slate-400">Deposit to (Cash Account)</Label><Select v-model="payoutToEdit.destination_asset_id"><SelectTrigger class="bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600"><SelectValue placeholder="Select destination account" /></SelectTrigger><SelectContent class="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200"><SelectGroup><SelectItem v-for="a in cashAssets" :key="a.id" :value="a.id">{{ a.name }}</SelectItem></SelectGroup></SelectContent></Select></div>
            <div class="space-y-2"><Label class="text-slate-600 dark:text-slate-400">Portfolio</Label><Select v-model="payoutToEdit.portfolio_id"><SelectTrigger class="bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600"><SelectValue placeholder="Select a portfolio" /></SelectTrigger><SelectContent class="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200"><SelectGroup><SelectItem v-for="p in portfoliosList" :key="p.id" :value="p.id">{{ p.name }}</SelectItem></SelectGroup></SelectContent></Select></div>
            <div class="flex items-center space-x-3 pt-4 border-t border-slate-200 dark:border-slate-700 mt-2"><Switch id="is-recurring" v-model:checked="payoutToEdit.is_recurring" /><Label for="is-recurring">This is a recurring income</Label></div>
            <div v-if="payoutToEdit.is_recurring" class="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200 dark:border-slate-700">
              <div class="space-y-2"><Label class="text-slate-600 dark:text-slate-400">Frequency</Label><Select v-model="payoutToEdit.recurring_frequency"><SelectTrigger class="bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600"><SelectValue placeholder="Select frequency" /></SelectTrigger><SelectContent class="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200"><SelectGroup><SelectItem v-for="f in frequencyOptions" :key="f" :value="f" class="capitalize">{{ f }}</SelectItem></SelectGroup></SelectContent></Select></div>
              <div class="space-y-2"><Label class="text-slate-600 dark:text-slate-400">End Date (Optional)</Label><Input type="date" v-model="payoutToEdit.recurring_end_date" class="bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600"/></div>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog :open="isAssignDialogOpen" @update:open="isAssignDialogOpen = $event">
        <DialogContent class="sm:max-w-md bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200">
          <DialogHeader><DialogTitle class="text-slate-900 dark:text-white">Assign Destination</DialogTitle><DialogDescription class="text-slate-500 dark:text-slate-400">Select a cash account to deposit this income into.</DialogDescription></DialogHeader>
          <div class="py-4 space-y-2"><Label class="text-slate-600 dark:text-slate-400">Deposit to (Cash Account)</Label><Select v-model="payoutToAssign.destination_asset_id"><SelectTrigger class="bg-slate-50 dark:bg-slate-700 border-slate-300 dark:border-slate-600"><SelectValue placeholder="Select destination account" /></SelectTrigger><SelectContent class="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200"><SelectGroup><SelectItem v-for="a in cashAssets" :key="a.id" :value="a.id">{{ a.name }}</SelectItem></SelectGroup></SelectContent></Select></div>
          <DialogFooter>
            <DialogClose as-child><Button type="button" variant="outline" class="border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white">Cancel</Button></DialogClose>
      ...
      <AlertDialog :open="isDeleteDialogOpen" @update:open="isDeleteDialogOpen = $event">
        <AlertDialogContent class="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-200">
          <AlertDialogHeader><AlertDialogTitle class="text-slate-900 dark:text-white">Confirm Deletion</AlertDialogTitle>
            <AlertDialogDescription class="text-slate-500 dark:text-slate-400">
              Are you sure you want to delete this income entry? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div v-if="payoutToDelete" class="py-4 text-sm text-slate-500 dark:text-slate-400 border-y border-slate-200 dark:border-slate-700 my-4">
